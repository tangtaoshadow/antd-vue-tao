...
tao.actions = {
    ...
    async getProjectList({ commit }) {
        ...
        await axios
            ...
            .catch(function (error) {
                obj.status = -1;
                TAO.my_console("error", "Tangtao：网络错误", error);
            });
        if (0 == obj.status) {
            // 数据成功返回 不考虑其他情况
        } else {
    ...
    async deleteProject({ state, commit }, payload) {
        ...
        await axios
            ...
            .catch(function (error) {
                obj.status = -1;
                TAO.consolelog("error", "Tangtao", error);
            });
        if (0 == obj.status) {
            // 数据成功返回 不考虑其他情况
        } else {
